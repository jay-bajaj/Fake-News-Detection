...
class dataset(Dataset):
    def __init__(self, path_to_glove='glove.6B.200d.txt',embedding_dim=200, prep_Data_from = 'train', purpose='train_model'):
        """
    ...
    def __getitem__(self, idx): # 1.25 Mark
        ...
        statement=[]
        justification=[]
        sentences=self.dataframe.iloc[idx,3]
        sentences=sentences.lower()
        statement.extend(word_tokenize(sentences))      

        sentences=self.dataframe.iloc[idx,15]
        sentences=sentences.lower()
        justification.extend(word_tokenize(sentences))

        credit_history=torch.zeros([5])
        ...
        vectorized_statement=torch.tensor(data=vectorized_statement, dtype=torch.float32)
        vectorized_justification=torch.tensor(data=vectorized_justification, dtype=torch.float32)
        credit_history[0]=self.dataframe.iloc[idx,12]
        credit_history[1]=self.dataframe.iloc[idx,11]
        credit_history[2]=self.dataframe.iloc[idx,9]
        credit_history[3]=self.dataframe.iloc[idx,10]
